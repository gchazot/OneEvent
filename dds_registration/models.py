...
class Booking(models.Model):
    ...
    def get_calendar_entry(self):
        ...
        cal.add("prodid", "-//dds_registration event entry//onevent//EN")
        cal.add("version", "2.0")
        ...
        cal_evt.add("uid", "event{0}-booking{1}@dds_registration".format(event.id, self.id))
    ...
    def send_calendar_invite(self):
        ...
        from_full = "{0} on behalf of {1} <{2}>".format(
            settings.dds_registration_SITE_BRAND,
            self.event.owner.get_full_name(),
            settings.dds_registration_CALENDAR_INVITE_FROM,
        )
