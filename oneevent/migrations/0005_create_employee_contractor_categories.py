...
def create_categoories(apps, _schema_editor):
    ...
    for event in Event.objects.all():
        ...
        if exceptions.exists():
            order += 1
            exc_category = event.categories.create(order=order, name="Exceptions",
                                                   price=event.price_for_contractors)
            for exc_group in exceptions:
                exc_category.groups1.add(exc_group)

        if employees.exists():
            order += 1
            emp_category = event.categories.create(order=order, name="Employees",
                                                   price=event.price_for_employees)
            for emp_group in employees:
                emp_category.groups1.add(emp_group)

        if contractors.exists():
            order += 1
            cnt_category = event.categories.create(order=order, name="Contractors",
                                                   price=event.price_for_contractors)
...
def delete_categories(apps, _schema_editor):
    ...
    Category.objects.filter(name__in=['Exceptions', 'Employees', 'Contractors']).delete()
